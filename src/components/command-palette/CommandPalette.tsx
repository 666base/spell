...
export function CommandPalette({
  ...
  const commands = useMemo<Command[]>(() => {
    ...
    // Settings and theme commands live at the bottom.
    baseCommands.push(
      {
        id: "settings",
        label: "Settings",
  ...
  // The palette mounts only while open, so focus after its first paint.
  useEffect(() => {
      ...
      <div role="dialog" aria-modal="true" aria-label="Command palette" className="relative w-full h-full max-h-108 max-w-2xl bg-bg/95 backdrop-blur-xl rounded-2xl shadow-[var(--shadow-surface)] overflow-hidden border border-border flex flex-col pointer-events-auto">
        ...
        <div className="border-b border-border flex-none">
          <input
            ...
            autoCapitalize="off"
            spellCheck={false}
            className="w-full px-4.5 py-3.5 text-[17px] bg-transparent outline-none text-text placeholder-text-muted/50 focus:bg-bg-muted/30 transition-colors"
          />
        </div>
