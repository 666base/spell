  ...
  const handleKeyDown = useCallback(
    (event: React.KeyboardEvent<HTMLDivElement>) => {
      ...
      const clampedWidth = Math.round(
        Math.min(Math.max(nextWidth, MIN_WIDTH), containerWidth),
      );
      const snappedPreset = PRESET_PX.find(
        (preset) =>
          Math.abs(clampedWidth - preset.px) < SNAP_THRESHOLD &&
          preset.px <= containerWidth,
      );
      if (snappedPreset) {
        setEditorWidth(snappedPreset.width);
      } else {
        setCustomEditorWidthPx(clampedWidth);
      }
    },
    [containerRef, getCurrentEditorWidth, setCustomEditorWidthPx, setEditorWidth],
  );
  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-10">
      ...
      <div
        ...
        aria-valuenow={Math.round(getCurrentEditorWidth())}
        aria-valuemin={MIN_WIDTH}
        aria-valuemax={containerRef.current?.clientWidth ?? MIN_WIDTH}
        tabIndex={0}
        className={cn(
          "absolute top-0 h-full w-3 cursor-col-resize pointer-events-auto group outline-none",
          isDragging && "z-20",
        )}
