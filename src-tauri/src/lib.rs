...
mod desktop {
        ...
        for component in rel.components() {
            match component {
                std::path::Component::ParentDir => {
                    return Err(
                        "Invalid note ID: parent directory references not allowed".to_string()
                    );
                }
                std::path::Component::CurDir => {
                    return Err(
                        "Invalid note ID: current directory references not allowed".to_string()
                    );
                }
    ...
    fn set_cloud_notes_folder(
        ...
        let valid_user_id = user_id.len() == 36
            && user_id
                .chars()
        ...
        {
            let mut app_config = state.app_config.write().expect("app_config write lock");
            app_config.cloud_user_id = Some(user_id);
            save_app_config(&app, &app_config).map_err(|e| e.to_string())?;
        }
    ...
    async fn apply_cloud_note(
        ...
        let timestamp = std::time::UNIX_EPOCH + Duration::from_secs(modified as u64);
        let file = std::fs::OpenOptions::new()
            .write(true)
            .open(&file_path)
            .map_err(|e| e.to_string())?;
        file.set_times(std::fs::FileTimes::new().set_modified(timestamp))
            .map_err(|e| e.to_string())?;
        ...
        let (final_id, file_path, old_id) = if let Some(existing_id) = id {
            ...
            let (dir_prefix, desired_id) = if let Some(pos) = existing_id.rfind('/') {
                ...
                (
                    ...
                    if existing_id.starts_with("journals/") {
                        existing_id.clone()
                    } else {
    ...
    fn validate_folder_path(path: &str) -> Result<(), String> {
        ...
        for component in rel.components() {
            match component {
                ...
                std::path::Component::CurDir => {
                    return Err(
                        "Invalid path: current directory references not allowed".to_string()
                    );
                }
                std::path::Component::RootDir | std::path::Component::Prefix(_) => {
                                        ...
                                        match std::fs::read_to_string(path) {
                                            Ok(content) => {
                                                ...
                                                let _ = search_index.index_note(
                                                    &note_id, &title, &content, modified,
                                                );
                                            }
                                            Err(_) => {
    ...
    async fn execute_ai_cli(
        ...
        let result = match tokio::time::timeout(timeout_duration, &mut task).await {
            Ok(join_result) => join_result
                .map_err(|e| format!("Failed to join {} blocking task: {}", cli_name, e))?,
            Err(_) => {
    ...
    fn try_select_in_notes_folder(app: &AppHandle, path: &Path) -> bool {
        ...
        let folder_path = PathBuf::from(&folder);
        let (canonical_file, canonical_folder) =
            match (path.canonicalize(), folder_path.canonicalize()) {
                (Ok(f), Ok(d)) => (f, d),
                _ => return false,
            };
